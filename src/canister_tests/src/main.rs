...
use ic_cdk::export::candid::{parser::value::IDLValue, IDLArgs};
...
fn test_storage_roundtrip() {
    ...
    let kv_store_wasm =  std::fs::read("../../internet_identity.wasm").unwrap();

    let nulls = vec![IDLValue::Null; 1];
    let args = IDLArgs::new(&nulls);
    let byts = args.to_bytes().unwrap();
    let canister_id = env
        .install_canister(kv_store_wasm.clone(), byts, None)
        .unwrap();
